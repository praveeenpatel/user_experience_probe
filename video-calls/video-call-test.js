...
async function startCalling(agentPage,nameCheck,widgetFrame) {

    //await agentPage.waitFor(10000);
    console.log("Entered to start calling",nameCheck);
    try {
    ...
    const conversationClick = await agentPage.waitForXPath(
        "//*[@class='virtualization-list-view']/li[1]/div/h3/span[contains(text(), '"+nameCheck+"')]"
    );
...
exports.openVisitor = async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < maxPages; i++) {
            ...
            if(status>=200 && visStatus>=200) {
                let [callStatus,callText] = await startCalling(agentPage,nameCheck,widgetFrame);
                if(callStatus >= 200) {
        ...
        const frames = closeConnection.widgetFrames;
        if(frames.length === maxPages) {
            for(let i=0;i<frames.length;i++) {
                let [endCallStatus] = await endCalls(frames[i]);
